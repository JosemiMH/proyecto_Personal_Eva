...
const Blog = () => {
  ...
  const BlogGrid = ({ articles, delay = 0 }: { articles: Article[], delay?: number }) => (
    <motion.div
        ...
        {archiveArticles.length > 0 && (
          <div className="mt-20">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h3 className="font-playfair text-3xl md:text-4xl font-bold text-charcoal">
                2025 {language === 'es' ? 'Archivo' : 'Archive'}
              </h3>
              <div className="h-px bg-gray-200 flex-grow"></div>
            </motion.div>
            <BlogGrid articles={archiveArticles} />
          </div>
        )}
      </div>
    </section>
